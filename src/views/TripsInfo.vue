<template>
  <div
    class="bg-gradient-to-r from-sky-200 to-sky-900 bg-cover bg-center bg-no-repeat flex items-center justify-center min-h-screen p-4"
  >
    <div
      class="lg:mx-auto md:mx-auto shadow-lg rounded-lg overflow-hidden w-full md:w-3/4 lg:w-1/2 m-auto"
    >
      <div
        class="grid text-center items-center shadow-[0_2px_10px_-3px_rgba(6,81,237,0.3)] rounded-xl bg-gradient-to-r from-yellow-100 to-yellow-50 overflow-hidden"
      >
        <!-- Form -->
        <ShipmentPop v-if="show" @done="Done"></ShipmentPop>
        <form
          class="p-6 space-y-6 flex flex-col items-center"
        ...
        >
          ...
          <div class="space-y-6 w-full md:w-3/4 lg:w-2/3">
            <div class="mx-auto">
              <input
                name="From"
                type="text"
                class="w-full bg-white border border-gray-300 text-gray-800 text-sm px-4 py-2.5 rounded-md focus:outline-blue-500"
                placeholder="From (City, Country)"
                v-model="From"
              />
              <span v-if="fromerror" class="text-red-500 text-sm">{{
                fromerror
              }}</span>
            </div>
            <div class="mx-auto">
              <input
                ...
                v-model="To"
              />
              <span v-if="toerror" class="text-red-500 text-sm">{{
                toerror
              }}</span>
            </div>
            <div class="mx-auto">
            ...
            <div class="mx-auto">
              <input
                ...
                v-model="weight"
              />
              <span v-if="weighterror" class="text-red-500 text-sm">{{
                weighterror
              }}</span>
            </div>
            <div class="mx-auto">
              <input
                ...
                v-model="userName"
              />
              <span v-if="usererror" class="text-red-500 text-sm">{{
                usererror
              }}</span>
            </div>
            <div class="mx-auto">
              ...
              />
              <span v-if="rewarderror" class="text-red-500 text-sm">{{
                rewarderror
              }}</span>
            </div>
          </div>
          ...
          <div
            ...
            :key="index"
            class="bg-blue-100 border border-gray-300 shadow-md shadow-grey-300 rounded-lg p-4 mt-7 w-full md:w-3/4 lg:w-2/3 mx-auto"
          >
          ...
          <div class="mt-12 w-full md:w-3/4 lg:w-2/3 mx-auto flex">
            <button
              type="button"
              @click="addMore"
              class="w-full py-3 px-4 tracking-wider text-sm text-white bg-gradient-to-r from-blue-800 to-blue-400 rounded-full hover:bg-blue-600 focus:outline-none"
            >
              Add More Shopping item
            </button>
            <button
              @click="goToConfirm"
              class="w-full py-3 px-4 text-white bg-gradient-to-r from-blue-800 to-blue-400 rounded-full"
            >
              Next
            </button>
          </div>
...
const validateForm = () => {
  if (!From.value || /^\d/.test(From.value)) {
    fromerror.value =
      "Please enter from Country & should not start with a number";
    return false;
